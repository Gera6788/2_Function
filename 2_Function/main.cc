...
constexpr uint16_t countPoint_X_Y = 30;
constexpr uint16_t countPoint_Z = 2;
constexpr uint16_t indexPoint_X_Y = Y_center / countPoint_X_Y;
constexpr uint16_t indexPoint_Z = Y_center / countPoint_Z;
constexpr uint32_t COL_BACKGROUND = 0xff003f3f;
constexpr uint32_t COL_DOT = 0x00FF00;
...
std::vector<int> get_Dot(double x, double y, double z) {
	double window_Xx = X_center + indexPoint_X_Y * (0 * cos(angle120) - x * sin(angle120));
	double window_Xy = Y_center - indexPoint_X_Y * (0 * sin(angle120) + x * cos(angle120));
	double window_Yx = X_center + indexPoint_X_Y * (0 * cos(angle240) - y * sin(angle240));
	double window_Yy = Y_center - indexPoint_X_Y * (0 * sin(angle240) + y * cos(angle240));
	double window_Zx = X_center;
	double window_Zy = Y_center - z * indexPoint_Z;
	int x_result = (int) (X_center + (window_Xx - X_center) + (window_Yx - X_center) + (window_Zx - X_center));
	int y_result = (int) (Y_center + (window_Xy - Y_center) + (window_Yy - Y_center) + (window_Zy - Y_center));
...
int main() {
	...
	for (auto &&h : horizon) {	h = IMG_HEIGHT;
	}


	for (double step_X = countPoint_X_Y; step_X > -countPoint_X_Y; step_X -= 0.65) {
		for (double step_Y = countPoint_X_Y; step_Y > -countPoint_X_Y; step_Y -= 0.05) {
	...
	for (double stepX = countPoint_X_Y; stepX > -countPoint_X_Y; stepX -= 0.05) {
		for (double stepY = countPoint_X_Y; stepY > -countPoint_X_Y; stepY -= 0.65) {
